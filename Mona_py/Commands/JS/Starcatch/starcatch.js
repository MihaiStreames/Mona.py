    ...
    starcatchChannelsJSON["channels"].forEach(channelID => {
        try {
            ...
            collector.on('collect', collectedMessage => {
                    if (first5Users.length < 5 && !collectedMessage.author.bot && collectedMessage.content.toLowerCase().includes("!catch")) {
                        main.commandsRan += 1;
                        // If user collected the star in another/the same server tell them that they can't collect another from the same run
                        if (usersThatCollected.includes(collectedMessage.author.id)) {
                            return collectedMessage.channel.send(`${collectedMessage.author} You already caught this ${chosenStar["type"]}!`)
                        }
            ...
            if (error) return message.channel.send(`${message.author} Something went wrong!`)
        }
    });
};
...
module.exports = {
    ...
    execute(message, args) {
            ...
            setup: () => {
                ...
                async function createNewFeed(message, isNew, existingChannel) {
                    ...
                    try {
                        ...
                        var fightersRole = await message.guild.roles.create({
                            data: {
                                name: '🌠 Adventurer\'s Guild',
                                color: '#808ccc',
                            }, reason: 'Role for people who want to be notified for new bosses',
                        });
        ...
        if (args[0] == "drop") {
            ...
            // Probably IDs of George, Mihai and Dave?
        }
    },
};